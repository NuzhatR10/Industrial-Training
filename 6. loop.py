...
# Assignment 1: Process user-provided temperature readings and print a warning for high temperatures

# Define the threshold for high temperatures
threshold = 30
...
num_readings = int(input("How many temperature readings do you want to input? "))

# Loop to take temperature readings as input from the user
...
# Dictionary of products with stock and minimum required levels
products = {
    "laptop": {"stock": 4, "min_required": 10},
    "smartphone": {"stock": 15, "min_required": 5},
    "tablet": {"stock": 2, "min_required": 8},
    "headphones": {"stock": 12, "min_required": 10}
}

# Process each product and check stock levels
...
for product, details in products.items():
    ...
    # Check if the product stock is below the minimum requirement
    if stock < min_required:
        print(f"⚠️ Restock Alert: {product} - Current stock: {stock}, Minimum required: {min_required}")
    else:
        print(f"✅ {product} is sufficiently stocked. Current stock: {stock}")

...
# Accessing elements
print("Element at row 1, column 2:", matrix[0][1])  # Accessing element 2
print("Element at row 3, column 1:", matrix[2][0])  # Accessing element 7

# Modifying elements
matrix[1][1] = 10  # Modifying the center element (5 -> 10)
print("Modified Matrix:", matrix)  # Directly printing the matrix

# Adding a new row
new_row = [10, 11, 12]
matrix += [new_row]  # Using list concatenation to add a new row
print("Matrix after adding a new row:", matrix)


# Accessing all elements without explicit iteration (list comprehensions)
all_elements = [element for row in matrix for element in row]
print("All elements in the matrix (flattened):", all_elements)

# Removing a specific row (e.g., the last row)
matrix.pop()  # Removes the last row
print("Matrix after removing the last row:")
for row in matrix:
    print(row)

# Sorting each row in the matrix (optional)
for i in range(len(matrix)):
    matrix[i].sort()
print("Matrix after sorting each row:")
for row in matrix:
    print(row)
...
# Creating a tuple with mixed data types (student's name, age, and grade)
student_data = ("Alice", 20, "A")

# Using the tuple as a key in a dictionary to store additional information
student_dict = {
    student_data: "Top Performer"
}

# Displaying the dictionary
print("Dictionary with student data as key:", student_dict)

# Accessing the value using the tuple key
student_status = student_dict[student_data]
...
# Creating a list of tuples with student names and their grades
students = [("Alice", 85), ("Bob", 78), ("Charlie", 92), ("Diana", 88)]

# Sorting the list by grades (ascending order)
